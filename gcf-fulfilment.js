...
const pubsubClient = pubsub({projectId: 'my-project-1497598125918'});
    ...
    .onRequest((request, response) => {
        ...
        const actionHandlers = {
            ...
            "add.number": () => {
                ...
                if (requestSource === googleAssistantRequest) {
                    let responseToUser = {
                        ...
                        // uncomment to enable
                        speech: `The sum of the numbers is ${total}`, // spoken response
                        displayText: `The sum of the numbers is ${total}` // displayed response
                    };
            ...
            "actor.search": () => {
                ...
                axios("https://anapioficeandfire.com/api/characters/?name=" + name).then(function (response) {

                    var actor = response.data[0].name,
                        aliases = response.data[0].aliases[0];
                    // Use the Actions on Google lib to respond to Google requests; for other
                    // requests use JSON
                    if (requestSource === googleAssistantRequest) {
                        let responseToUser = {
                            ...
                            // googleOutputContexts: ['weather', 2, { ['city']: 'rome' }], // Optional,
                            // uncomment to enable
                            speech: `${firstName} is a great actor and know as ${aliases}`, // spoken response
                            displayText: `${firstName} is a great actor and know as ${aliases}` // displayed response
                        };
                        sendGoogleResponse(responseToUser);
                        // sendGoogleResponse(`${actor.name} is a great actor and know as ${aliases}`);
                        // // Send simple response to user
                    ...
                    .catch(function () {
                        if (requestSource === googleAssistantRequest) {
                            let responseToUser = {
                                ...
                                // googleOutputContexts: ['weather', 2, { ['city']: 'rome' }], // Optional,
                                // uncomment to enable
                                speech: `no one found`, // spoken response
                                displayText: "no one found" // displayed response
                            };
                            sendGoogleResponse(responseToUser);
                            // sendGoogleResponse(`${actor.name} is a great actor and know as ${aliases}`);
                            // // Send simple response to user
            ...
            default: () => {
                fetch("https://anapioficeandfire.com/api/characters/?name=" + parameters.actorname)
                    .then(function (response) {
                        var actor = response.data.actor,
                        ...
                        if (requestSource === googleAssistantRequest) {
                            let responseToUser = {
                                ...
                                // googleOutputContexts: ['weather', 2, { ['city']: 'rome' }], // Optional,
                                // uncomment to enable
                                speech: `${actor.name} is a great actor and know as ${aliases}`, // spoken response
                                displayText: `${actor.name} is a great actor and know as ${aliases}` // displayed response
                            };
                            sendGoogleResponse(responseToUser);
                            // sendGoogleResponse(`${actor.name} is a great actor and know as ${aliases}`);
                            // // Send simple response to user
        ...
        attachments: [
            {
                ...
                text: "This is an attachment.  Text in attachments can include 'quotes' and most other " +
                    "unicode characters including emoji 📱.  Attachments also upport line\nbreaks.",
